...
struct WalletTransactionDto
{
    ...
    uint64_t unlockTime;
    std::string extra;
    boost::optional<Crypto::SecretKey> secretKey = Qwertycoin::NULL_SECRET_KEY;
};
...
void WalletSerializerV1::loadWallets(Common::IInputStream &source, CryptoContext &cryptoContext)
{
    ...
    for (uint64_t i = 0; i < count; ++i) {
        ...
        if (i == 0) {
            isTrackingMode = dto.spendSecretKey == Qwertycoin::NULL_SECRET_KEY;
        } else if ((isTrackingMode && dto.spendSecretKey != Qwertycoin::NULL_SECRET_KEY)
                   || (!isTrackingMode && dto.spendSecretKey == Qwertycoin::NULL_SECRET_KEY)) {
        ...
        if (dto.spendSecretKey != Qwertycoin::NULL_SECRET_KEY) {
