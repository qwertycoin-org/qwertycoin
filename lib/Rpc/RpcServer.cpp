...
template <typename Command>
RpcServer::HandlerFunction httpMethod(
    bool (RpcServer::*handler)(typename Command::request const&, typename Command::response&))
{
    return [handler](RpcServer* obj, const HttpRequest& request, HttpResponse& response) {
...
> RpcServer::s_handlers = {
...

    {
        ...
        {
            jsonMethod<COMMAND_RPC_GET_BLOCK_DETAILS_BY_HASH>(&RpcServer::onGetBlockDetailsByHash),
            true
        }
    },{
...
bool RpcServer::onGetBlockDetailsByHash(
...
bool RpcServer::on_get_index(const COMMAND_HTTP::request& req, COMMAND_HTTP::response& res)
{
    ...
    const std::string uptime_str = std::to_string(
        (unsigned int)floor(uptime / 60.0 / 60.0 / 24.0))
            + "d "
            + std::to_string((unsigned int)floor(fmod((uptime / 60.0 / 60.0), 24.0)))
            + "h "
            + std::to_string((unsigned int)floor(fmod((uptime / 60.0), 60.0)))
            + "m "
            + std::to_string((unsigned int)fmod(uptime, 60.0))
            + "s";
        ...
        COMMAND_RPC_GET_TRANSACTIONS_BY_HEIGHTS::response &res)
{
    try {
        ...
        logger(DEBUGGING) << "Found " << txs.size() << "/" << vh.size() << " transactions on the blockchain.";
