...
#define MDBValSet(var, val) MDB_val var = {sizeof(val), (void *)&val}
	...
	const std::string lmdbError(const std::string &cErrorString, int iDBRes)
	{
...
#define TXN_BLOCK_PREFIX()
	...
	FMdbTxnSafe::FMdbTxnSafe(const bool bCheck) : pTxn(nullptr), pThreadInfo(nullptr), pCheck(bCheck)
	{
		if (bCheck) {
	...
	void FMdbTxnSafe::abort()
	{
		...
		mLogger(INFO, BRIGHT_CYAN) << "FMdbTxnSafe: abort()";
		if (pTxn != nullptr) {
			mdb_txn_abort(pTxn);
			pTxn = nullptr;
		}
		else {
			mLogger(INFO) << "WARNING: FMdbTxnSafe: abort() called, but pTxn is NULL";
		}
	}
	...
	bool BlockchainLMDB::blockReadTxnStart(MDB_txn **sMdbTxn, FMdbTxnCursors **sCurs) const
	{
		...
		if (!(sTInfo = mTInfo.get()) || mdb_txn_env(sTInfo->sTReadTxn) != mDbEnv) {
			...
			if (auto iMdbRes = lmdbTxnBegin(mDbEnv, NULL, MDB_RDONLY, &sTInfo->sTReadTxn)) {
								  ...
								  const Crypto::Hash &sBlockHash)
	{
		...
		checkOpen();
		{
			...
			FMdbValCopy<blobData> cBlob(blockToBlob(block));
									   ...
									   const uint64_t &uUnlockTime)
	{
		...
		mLogger(DEBUGGING, BRIGHT_CYAN) << "BlockchainLMDB::" << __func__
										<< ". !sCursor->sTxcOutputTransactions"
										<< !sCursor->sTxcOutputTransactions;
		mLogger(DEBUGGING, BRIGHT_CYAN) << "BlockchainLMDB::" << __func__
										<< ". !sCursor->sTxcOutputAmounts"
										<< !sCursor->sTxcOutputAmounts;

		CURSOR(OutputTransactions)
		CURSOR(OutputAmounts)

		if (sTxOutput.target.type() != typeid(TransactionOutputTarget)) {
			throw (DB_ERROR("Wrong output type: expected TransactionOutputTarget"));
		}

		FOutTx sOutTx = {uNumOutputs, sTxHash, uIndex};
		MDBValSet(sValOutTx, sOutTx);
		result = mdb_cursor_put(sCurOutputTransactions, (MDB_val *) &cZeroKVal, &sValOutTx, MDB_APPENDDUP);
