 ...
 */
	...
	public:
		 ...
		 * @return
		 */
		virtual CryptoNote::difficulty_type getBlockCumulativeDifficulty(const uint64_t &uHeight) const = 0;
		 ...
		 */
		virtual bool getTransaction(const Crypto::Hash &sHash, CryptoNote::Transaction &sTransaction) const;
		 ...
		 * @return True iff the transaction was found
		 */
		virtual bool getTransactionBlob(const Crypto::Hash &sHash, CryptoNote::blobData &sTransactionBlob) const = 0;
		...
		// TODO: this should perhaps be (or call) a series of functions which progressively update through version updates
		/**
		...
		mutable std::recursive_mutex pSyncronizationLock; // A lock, currently for when BlockchainLMDB needs to resize the backing db file
	};
