...
namespace CryptoNote {
	...
	{
		if (cDBType == "lmdb") {
			return new BlockchainLMDB(false, logger);
		}
		else {
			return NULL;
		}
	...
	CryptoNote::Transaction BlockchainDB::getTransaction(const Crypto::Hash &sHash) const
	{
		...
		if (!getTransaction(sHash, sTransaction)) {
			throw (TX_DNE(
					std::string("tx with hash ").append(Common::podToHex(sHash)).append(" not found in db").c_str()));
		}
	...
	void BlockchainDB::showStats()
	{
		std::cout << std::endl
				  ...
				  << "mNumCalls: " << mNumCalls
				  << std::endl
				  << "time_blk_hash: " << mTimeBlockHash << "ms"
				  << std::endl
				  << "time_tx_exists: " << gTimeTxExists << "ms"
				  << std::endl
				  << "time_add_block1: " << mTimeAddBlock << "ms"
				  << std::endl
				  << "time_add_transaction: " << mTimeAddTransaction << "ms"
				  << std::endl
				  << "time_commit1: " << gTimeCommit << "ms"
				  << std::endl
				  << "*********************************"
				  << std::endl;
	}
