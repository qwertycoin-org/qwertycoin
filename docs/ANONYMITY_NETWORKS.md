# Anonymity Networks with Qwertycoin

Currently only Tor and I2P have been integrated into Qwertycoin. The usage of
...
is made or until `qwertycoind` is shutdown and restarted with only public
connections enabled.

Anonymity networks can also be used with `qwertycoin-wallet-cli` and
`qwertycoin-wallet-rpc` - the wallets will connect to a daemon through a proxy. The
...
`--tx-proxy` which tells `qwertycoind` the ip/port of a socks proxy provided by a
...
which tells `qwertycoind` that ".onion" p2p addresses can be forwarded to a socks
...
```
--add-exclusive-node rveahdfho7wo4b2m.onion:8196
--add-peer rveahdfho7wo4b2m.onion:8196
```
...
`--anonymous-inbound`. This option tells `qwertycoind` the inbound address, network
type, and max connections:

```
--anonymous-inbound rveahdfho7wo4b2m.onion:8196,127.0.0.1:8196,25
--anonymous-inbound cmeua5767mz2q5jsaelk2rxhf67agrwuetaso5dzbenyzwlbkg2q.b32.i2p,127.0.0.1:30000
```

which tells `qwertycoind` that a max of 25 inbound Tor connections are being
received at address "rveahdfho7wo4b2m.onion:8196" and forwarded to `qwertycoind`
localhost port 8196, and a default max I2P connections are being received at
address "cmeua5767mz2q5jsaelk2rxhf67agrwuetaso5dzbenyzwlbkg2q.b32.i2p" and
forwarded to `qwertycoind` localhost port 30000.
...
`qwertycoind` RPC port - which is independent from the configuration for incoming
P2P anonymity connections. The anonymity network (Tor/i2p) is
[configured in the same manner](#configuration), except the localhost port
must be the RPC port (typically 8197 for mainnet) instead of the p2p port:

```
HiddenServiceDir /var/lib/tor/data/qwertycoin
HiddenServicePort 8197 127.0.0.1:8197
```
...
```
HiddenServiceDir /var/lib/tor/data/qwertycoin
HiddenServicePort 8196 127.0.0.1:8196
```

This will store key information in `/var/lib/tor/data/qwertycoin` and will forward
"Tor port" 8196 to port 8196 of ip 127.0.0.1. The file
`/usr/lib/tor/data/qwertycoin/hostname` will contain the ".onion" address for use
...
system clock is fairly accurate - many peers on the Qwertycoin network should have
similar timestamps.
...
### Intermittent Qwertycoin Syncing

If a user only runs `qwertycoind` to send a transaction then quit, this can also
be used by an ISP to link a user to a transaction.

#### Mitigation

Run `qwertycoind` as often as possible to conceal when transactions are being sent.
...
`qwertycoind` currently selects two outgoing connections every 5 minutes for
...
`qwertycoind` will have rotated its selected outgoing connections several times in
...
circuits is observable by the ISP. Instead, `qwertycoind` should likely disconnect
