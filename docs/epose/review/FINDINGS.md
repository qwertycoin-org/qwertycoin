...
| F16 | **CO-07 state machine implemented; integration open / P1** | V2 registration, renewal, descriptor update, deregistration, and service-key recovery are sequence/predecessor-bound, future-effective, and dual-authorized. Operator-key rotation is intentionally unsupported. | Finalize record codec, key custody/migration UX, envelope integration, and durable rollback in CO-08. |
| F17 | **CO-08 journal primitive implemented; LMDB gate open / P1** | A schema/parameter-bound, checksummed, bounded-undo checkpoint journal now fails atomically and rebuilds from canonical history. It is not yet stored in the blockchain LMDB transaction. | Add LMDB tables/adapter, startup tip validation, object reconstruction, crash injection, migration rollback, and pruning proof before activation. |
