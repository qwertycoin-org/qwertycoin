...
| F06 | **Shared carrier and codecs implemented; delivery open / P0** | Miner and fee-funded transactions use the same version-gated `tx_extra` adapter and envelope budgets. All registered payload types now have bounded atomic codecs. Miner batching, wallet submission, queue fairness, and measured inclusion delay are not integrated. | Connect both producers and the semantic batch adapter; prove capacity, fairness, inclusion delay, and starvation resistance. |
| F07 | **HF18 carrier and typed codecs implemented; state integration open / P0** | Dedicated canonical `0x05` / `QEP2` framing is a real length-delimited `tx_extra` variant. The default parser and HF17 reject it; the adapter opts in only for reserved HF18. Admission, receipt, dual-authorized lifecycle and scoped-payment-proof codecs validate cryptographic context atomically. | Add the shared differential corpus and connect the handler without scheduling HF18 until every gate closes. |
