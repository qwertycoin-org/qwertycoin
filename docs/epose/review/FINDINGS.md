...
| F07 | **HF17/v2 carrier, codecs and canonical handler implemented; integrated adversarial evidence open / P0** | Dedicated canonical `0x05` / `QEP2` framing is a length-delimited `tx_extra` variant. QWC block version 17 starts the hardened EPoSE-v2 state machine; inherited version 16 and legacy-v1 objects remain invalid. A later separately scheduled block version continues the same parameter-bound state without reset, while the hardfork schedule still rejects unscheduled versions. The canonical coordinator applies bounded records atomically, validates the scoped Coinbase proof and commits the resulting state with the block. | Complete differential/fuzz, malformed-block, multi-node and independent consensus evidence; launch remains blocked until every gate closes. |
...
| F10 | **Rehearsal economics selected; E2E evidence open / P0** | The embedded candidate selects 1,000 BPS subsidy-only rewards, actual-issued-subsidy accounting and miner fallback for an empty qualification set. | Prove emission continuity, payout-fork behavior and wallet outcomes; retain the qualification-suppression incentive in the final risk assessment. |
...
| F14 | **Rehearsal bounds embedded; measured limits open / P0** | Envelope/block accumulators and a population ceiling are consensus-bound; relay queues have complete total/per-class rehearsal limits with deterministic expiry and selection. | CO-05/09: worst-valid-block and sustained-stream benchmarks, final limit selection, cached selection, and supported-population evidence. |
