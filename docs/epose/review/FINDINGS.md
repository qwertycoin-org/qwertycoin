...
| F06 | **CO-05 parser primitive implemented; delivery open / P0** | Carrier-neutral multi-envelope parsing exists, but miner batching, fee-funded wallet submission, queue fairness, and measured inclusion delay are not integrated. | Connect both carriers after payload codecs; prove capacity, fairness, inclusion delay, and starvation resistance. |
| F07 | **CO-05 envelope implemented; activation integration open / P0** | Dedicated canonical `0x05` / `QEP2` framing supports bounded records without extending the legacy nonce. It remains deliberately unrecognized by HF17 generic `tx_extra`. | Add version-aware HF18 integration and finalized record codecs/vectors without changing historical parsing. |
