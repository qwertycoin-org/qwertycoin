...
| F02 | **Confirmed / P0** | `chain_state.cpp` selects from `registry.active_nodes(attestation.epoch)` at application time; no immutable pre-seed snapshot exists. | CO-01/02: normative cutoff and frozen snapshot used by admission, vote validation, qualification, and payout. |
| F03 | **Confirmed design exposure / P0** | Registration and selection use current epoch context; identities can be admitted without a distinct future-epoch lease and pre-seed descriptor cutoff. | CO-02/03: bind identity, descriptor, target epoch, and admission context before seed revelation; add grinding simulation. |
...
| F08 | **Confirmed / P0** | `reward_source_epoch_for_height()` maps epoch 0 to itself; there is no explicit settlement cutoff separating final evidence from next payouts. | CO-01/02: warm-up, evidence deadline, closed qualification set, and boundary/reorg vectors in a future version. |
...
| F13 | **Partially covered / P0** | Transaction batches are atomic and state hash sorting is deterministic. Registrations precede attestations; duplicate/conflict and reward read-state rules are not a complete normative transition. | CO-01/08: canonical transition, version rules, duplicate semantics, reference model, crash/rebuild equivalence. |
...
| F20 | **Confirmed / P0** | Public `main`, the running `d5e6cf7de` binary, observed live HF17 chain, and release identity are not reconciled. | CO-00/01/11: release manifest, source-to-binary proof, chain-preserving future activation, public specification before activation. |
