...
| F06 | **Shared carrier/codecs and fair local queue implemented; delivery open / P0** | Miner and fee-funded transactions use the same version-gated `tx_extra` adapter and envelope budgets. All registered payload types have bounded atomic codecs. A deadline queue reserves item/byte capacity and template shares independently for enrollment and evidence. Miner/wallet producers and measured inclusion delay are not integrated. | Connect both producers and the semantic batch adapter; prove capacity, inclusion delay, and starvation resistance under sustained load. |
| F07 | **HF18 carrier, codecs and semantic batch implemented; handler open / P0** | Dedicated canonical `0x05` / `QEP2` framing is a real length-delimited `tx_extra` variant. The default parser and HF17 reject it; the adapter opts in only for reserved HF18. Admission, receipt, dual-authorized lifecycle and scoped-payment-proof codecs validate cryptographic context atomically. The shared semantic adapter applies lifecycle/membership changes to a temporary copy and rejects any invalid suffix without retaining a prefix. | Add the shared differential corpus and connect the canonical block/coinbase handler without scheduling HF18 until every gate closes. |
...
| F13 | **Atomic semantic transaction batches implemented; persistent integration open / P0** | Envelope parse/encode and transaction aggregation expose outputs only on success. The semantic adapter applies ordered lifecycle, admission and receipt records to copied state and commits only a wholly valid transaction. Canonical journal restore rejects duplicate checkpoints and requires exact decode/re-encode identity. | CO-08: connect block-atomic semantic state to same-transaction LMDB state and prove crash recovery/replay equivalence. |
