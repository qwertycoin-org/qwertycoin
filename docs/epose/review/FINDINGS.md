...
| F07 | **HF17/v2 carrier, codecs and semantic batch implemented; handler open / P0** | Dedicated canonical `0x05` / `QEP2` framing is a real length-delimited `tx_extra` variant. Exact QWC block version 17 permits the hardened EPoSE-v2 envelope; inherited version 16, unscheduled version 18, and legacy-v1 objects do not. Admission, receipt, dual-authorized lifecycle and scoped-payment-proof codecs validate cryptographic context atomically. The shared semantic adapter applies lifecycle/membership changes to a temporary copy and rejects any invalid suffix without retaining a prefix. | Add the shared differential corpus and connect the canonical HF17 block/coinbase handler; launch remains blocked until every gate closes. |
