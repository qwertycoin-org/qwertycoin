...
| F06 | **Shared carrier adapter implemented; delivery open / P0** | Miner and fee-funded transactions now use the same version-gated generic `tx_extra` adapter and envelope budgets. Miner batching, wallet submission, queue fairness, and measured inclusion delay are not integrated. | Complete remaining payload codecs and connect both producers; prove capacity, fairness, inclusion delay, and starvation resistance. |
| F07 | **HF18 carrier and receipt codec implemented; state integration open / P0** | Dedicated canonical `0x05` / `QEP2` framing is a real length-delimited `tx_extra` variant. The default parser and HF17 reject it; the adapter opts in only for reserved HF18. The 402-byte receipt codec validates context and both signatures atomically. | Complete the other typed codecs/shared corpus and connect the handler without scheduling HF18 until every gate closes. |
...
| F11 | **CO-06 scoped-proof candidate implemented; review open / P0** | V2 can bind prescribed one-time outputs using public reward keys, a recipient-specific derivation, and the inherited transaction-proof primitive without a private view key. HF17 disclosure remains irreversible. | Independent cryptographic review plus CO-07 fresh-wallet lifecycle/migration are required before activation. |
...
| F13 | **Corrected primitives; storage integration open / P0** | Envelope parse/encode and transaction aggregation now expose outputs only on success. Canonical journal restore rejects duplicate checkpoints and requires exact decode/re-encode identity. State hash sorting is deterministic. | CO-05/08: connect typed records and same-transaction persistent state; prove crash recovery and replay equivalence. |
| F14 | **CO-05 structural budgets implemented; measured limits open / P0** | Envelope and block accumulators bound bytes, records, signatures, and admission operations with fail-atomic charging. Limits are caller-supplied and the active manifest remains unset; selection/probe costs are not yet covered. | CO-05/09: worst-valid-block benchmarks, final manifest values, cached selection, and supported-population contract. |
...
| F16 | **Corrected state machine; integration open / P1** | Historical lookup applies the selected historical record rather than the latest record. Deregistration is terminal; later ordinary renewal cannot rewrite E5. Online service and offline operator keys must be distinct. | Derive effective epochs from canonical inclusion height; finalize codec, custody UX, envelope integration, and durable rollback. |
| F17 | **Corrected journal primitive; LMDB gate open / P1** | Restore/rebuild reject invalid configuration even for empty inputs; canonical images reject valid-checksum duplicate checkpoints and must round-trip byte-exactly. The digest remains a local journal digest, not a consensus state root. | Add LMDB tables/adapter, startup tip validation, canonical state commitment, crash injection, migration rollback, and pruning proof before activation. |
...
| F20 | **Manifest/release evaluator corrected; deployment open / P0** | The typed schema rejects Boolean integers, invalid relationships and test fixtures for release use. A separate immutable 13-ID policy requires candidate-bound artifact hashes. No future activation-block hash is a preactivation input. Public source and deployed binary identity remain unreconciled. | CO-11: source-to-binary proof, final signed activation manifest, public specification, and exact deployment reconciliation before activation. |
