...
for node in $(seq 0 "$((EPOSE_SERVICE_NODE_COUNT - 1))"); do
  ...
  if [[ "${logs}" != *"Submitted EPoSE-v2 lifecycle and admission for epoch 1"* ]]; then
    echo "Admission proof is not complete for node ${node}" >&2
    exit 1
  fi
done

printf 'resuming-after-admission at=%s\n' "$(date -u +%FT%TZ)" >"${status_file}"
