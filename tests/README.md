...
Core tests take longer than any other Qwertycoin tests, due to the high amount of computational work involved in validating core components.
...
To run only Qwertycoin's core tests (after building):
...
To run only Qwertycoin's crypto tests (after building):
...
```bash
qwertycoind --regtest --offline --fixed-difficulty 1
```
...
Open the wallet file with `qwertycoin-wallet-rpc` with RPC port 8198. Finally, start tests by invoking ./blockchain.py or ./speed.py
...
To run only Qwertycoin's hash tests (after building):
...
To run only Qwertycoin's performance tests (after building):
...
To run only Qwertycoin's unit tests (after building):
