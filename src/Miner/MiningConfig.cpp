...
MiningConfig::MiningConfig() : help(false)
{
    cmdOptions.add_options()("help,h", "produce this help message and exit")(
            ...
            "daemon-host", po::value<std::string>()->default_value(DEFAULT_DAEMON_HOST),
            "Daemon host")(
            "daemon-rpc-port",
            ...
            "Mining threads count. Must not be greater than you concurrency level. Default value "
            "is your hardware concurrency level")(
            "scan-time", po::value<size_t>()->default_value(DEFAULT_SCANT_PERIOD),
            "Blockchain polling interval (seconds). How often miner will check blockchain for "
            "updates")("log-level", po::value<int>()->default_value(1), "Log level. Must be 0..5")(
            ...
            "Timestamp step for each subsequent block. May be set only if --first-block-timestamp "
            "has been set."
...
void MiningConfig::parse(int argc, char **argv)
{
    ...
    if (!options["daemon-address"].empty()) {
        if (!options["daemon-host"].defaulted() || !options["daemon-rpc-port"].defaulted()) {
            throw std::runtime_error("Either --daemon-host or --daemon-rpc-port is already "
                                     "specified. You must not specify --daemon-address");
        }
    ...
    if (threadCount == 0 || threadCount > CONCURRENCY_LEVEL) {
        throw std::runtime_error("--threads option must be 1.."
                                 + std::to_string(CONCURRENCY_LEVEL));
    }
    ...
    if (!options["block-timestamp-interval"].defaulted()
        ...
        throw std::runtime_error("If you specify --block-timestamp-interval you must specify "
                                 "--first-block-timestamp either");
    }
