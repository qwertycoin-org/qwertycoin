...
void Configuration::initOptions(boost::program_options::options_description &desc)
{
    desc.add_options()("bind-address", po::value<std::string>()->default_value("127.0.0.1"),
            ...
            "mnemonic-seed", po::value<std::string>(),
            "generate a container with this mnemonic seed")(
            "deterministic",
            "generate a container with deterministic keys. View key is generated from spend key of "
            "the first address")("daemon,d", "run as daemon in Unix or as service in Windows")
#ifdef _WIN32
            ("register-service", "register service and exit (Windows only)")(
                    ...
                    ("log-file,l", po::value<std::string>(),
                     "log file")("server-root", po::value<std::string>(),
                                 "server root. The service will use it as working directory. Don't "
                                 "set it if don't want to change it")(
                            "log-level", po::value<size_t>(),
                            "log level")("address", "print wallet addresses and exit");
}
...
void Configuration::init(const boost::program_options::variables_map &options)
{
    ...
    if (registerService && unregisterService) {
        throw ConfigurationError("It's impossible to use both \"register-service\" and "
                                 "\"unregister-service\" at the same time");
    }
